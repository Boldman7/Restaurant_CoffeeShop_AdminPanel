...
class Admin extends CI_Controller
{
   ...
   public function allOrder()
    ...
    public function addUsage()
    {
        if(isset($_SESSION['name'])) 
        {   
            $usage= $this->Api_model->getAllData('usage');
            $data['company']= $this->Api_model->getAllData('company');
            $data['supplier']=$this->Api_model->getAllData('supplier_info');
            $data['product_info']=$this->Api_model->getAllData('product_info');
            $data['measurement_info']=$this->Api_model->getAllData('measurement_info');
    ...
    function addUsageAction() 
    {
       if(isset($_SESSION['name'])) 
        {
            ...
            $product = $this->Api_model->getSingleRow('product_info',array('pro_pub_id'=>$data['pro_pub_id']));
             ...
             $this->Api_model->insert('usage', $data);
       ...
       public function allpurechase()
       {
        ...
        {   
            ...
            if($date !="" && $date1 !="")
            {
                $data['purechase_info']= $this->Api_model->getDataBetween('purechase_info',$date,$date1);
            }
            else
            {
                $data['purechase_info']= $this->Api_model->getAllData('purechase_info');
            }
     ...
     if(isset($_SESSION['name'])) 
       ...
       if(isset($_SESSION['name'])) 
        ...
        public function editMenu(){
            ...
            $data['cat']=$this->Api_model->getAllData('food_cat');
    ...
    function report() 
    {
       if(isset($_SESSION['name'])) 
        {   
            ...
            $data['purechase_info']=$this->Api_model->getAllBetween(strtotime($today)*1000,strtotime($yesterday)*1000,'purechase_info');

            $data['monthly_purchase']=$this->Api_model->getAllBetween(strtotime($today)*1000,strtotime($month)*1000,'purechase_info');
            $data['yearly_purchase']=$this->Api_model->getAllBetween(strtotime($today)*1000,strtotime($yearly)*1000,'purechase_info');
            $data['company']= $this->Api_model->getAllData('company');
            $data['page']='purechase information';
            $this -> load -> view('common/head.php');
            $this -> load -> view('common/sidebar.php', $data);
